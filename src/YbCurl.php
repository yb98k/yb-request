...
class YbCurl
{
    ...
    public function __construct($allRequestData, $callBack = null, $curlCount = 5)
    {
        ...
        static::$curlHandler    = new Curl( $callBack );
    }
    ...
    public static function initRequestData($method = 1)
    {
        ...
        if(self::getDimensionality(static::$allRequestData) == 2) {
            ...
            $request = new CurlRequest(
                static::$allRequestData['url'],
                $method,
                static::$allRequestData['requestData'] ?? [],
                static::$allRequestData['header'] ?? [],
                static::$allRequestData['options'] ?? []
            );
